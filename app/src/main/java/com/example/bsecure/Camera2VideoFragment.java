...
public class Camera2VideoFragment extends Fragment
    ...
    private final int[] frameCount = new int[1];
    ...
    private void startBackgroundThread() {
        ...
        mBackgroundThread2 = new HandlerThread("ImageReader");
    ...
    private void openCamera(int width, int height) {
        ...
        try {
            ...
            imageReader = ImageReader.newInstance(mVideoSize.getWidth(), mVideoSize.getHeight(), ImageFormat.JPEG, 1);

            imageReader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() {
                @Override
                public void onImageAvailable(ImageReader reader) {
                    Image f = reader.acquireNextImage();
                    frameCount[0]++;
                    Log.d("imagereader", "Read image " + frameCount[0]);
                    f.close();
                }
            }, mBackgroundHandler2);
    ...
    private void startPreview() {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                configs = configureSession(mCameraDevice, surfaces);
            }
    ...
    private void updatePreview() {
        ...
        try {
            ...
            thread.start();
            mPreviewSession.setRepeatingRequest(mPreviewBuilder.build(), capCallback, mBackgroundHandler);
        } catch (CameraAccessException e) {
            e.printStackTrace();
        }
    }
    @RequiresApi(api = Build.VERSION_CODES.TIRAMISU)
    private ArrayList configureSession(CameraDevice device, List<Surface> targets) {
        ArrayList configs = new ArrayList();
        int streamUseCase = CameraMetadata
